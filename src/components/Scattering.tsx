...
const RedScatterForBlue = keyframes`
    from {
        transform: translate3d(315px, 315px, 0)
      }
    to {
        opacity: 1;
...
const blueElements: Point[] = [];
const redElements: Point[] = [];
...
export const DaytimeSky = styled("div")`
  ...
  animation-fill-mode: forwards;
  animation-play-state: ${(props: { runAnimation: boolean }) =>
    props.runAnimation ? "running" : "paused"};
`;
    ...
    return (
      <>
        ...
        {redElements.map((redElement, i) => (
          <ScatterElementRedForBlue
            // x={redElement.xPos}
            // y={redElement.yPos}
            key={i}
            style={{
              //   left: redElement.xPos,
              //   top: redElement.yPos,
              transform: `translate3d(${redElement.xPos}, ${
                redElement.yPos
              }, 0)`,
          ...
          blueElements.map((blueElement, i) => (
            <ScatterElementBlue
              ...
              style={{
                ...
                left: blueElement.xPos,
                top: blueElement.yPos,
                animationPlayState: this.props.runAnimation
                  ? "running"
                  : "paused"
              }}
            />
    ...
    return (
      <>
        ...
        {pollutedElements.map((pollutedElement, i) => (
          <ScatterElementPolluted
            key={i}
            style={{
              left: pollutedElement.xPos,
              top: pollutedElement.yPos,
              animationPlayState: this.props.runAnimation ? "running" : "paused"
            }}
          />
        ))}
